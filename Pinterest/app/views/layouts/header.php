...
if (isset($_SESSION['user']['id']) && isset($dbConfig) && is_array($dbConfig)) {
    try {
        ...
        $messageModel = new \App\Models\Message($db);
        $messageUnreadCount = $messageModel->countUnreadMessagesForUser((int) $_SESSION['user']['id']);
    } catch (\Throwable $e) {
        $notificationCount = 0;
        $messageUnreadCount = 0;
    }
...
<header class="topbar">
  ...
  <?php 
    // Lấy role hiện tại
    $userRole = isset($_SESSION['user']['role']) ? strtolower((string)$_SESSION['user']['role']) : 'user';
    ?>
    ...
    <?php if ($userRole !== 'admin'): ?>
        ...
        <?php if (isset($_SESSION['user'])): ?>
            ...
            <a class="nav-pill nav-pill-message" href="index.php?r=message/inbox">
                Tin nhắn
    ...
    <?php if ($userRole === 'admin'): ?>
        <a class="nav-pill text-danger font-weight-bold" href="index.php?r=admin/dashboard">
            Quản trị 
        </a>
    <?php endif; ?>
</nav>
