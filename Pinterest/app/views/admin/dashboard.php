...
<?php else: ?>
                ...
                <?php foreach ($reportedPins as $pin): ?>
                    <tr>
                        ...
                        <td style="width: 130px;">
                            <?php $rawImg = trim((string) ($pin['image_url'] ?? '')); ?>
                            <?php if ($rawImg !== ''): ?>
                                <img src="<?= htmlspecialchars('index.php?r=admin/pinImage&pin_id=' . (int) $pin['id']) ?>" alt="" class="img-fluid rounded" style="max-height: 100px; object-fit: cover;">
                            <?php else: ?>
    ...
    <form method="post" action="index.php?r=admin/deletePin" onsubmit="return confirm('Xóa pin này khỏi toàn hệ thống?');" style="display:inline-block;">
        <input type="hidden" name="pin_id" value="<?= (int) $pin['id'] ?>">
        <button class="btn btn-danger btn-sm" type="submit">Xóa pin</button>
    </form>

    <a href="index.php?r=admin/rejectReport&pin_id=<?= (int)$pin['id'] ?>" 
       class="btn btn-secondary btn-sm" 
       onclick="return confirm('Bạn có chắc muốn bỏ qua tất cả báo cáo cho Pin này?')">
       Từ chối
    </a>
</td>
                    </tr>
