...
{
    // Thêm 'role' vào danh sách SELECT ở cả 2 câu query
    $sql = 'SELECT id, username, name, avatar, email, password_hash, role 
            FROM users 
            WHERE email = :email
    ...
    if (!$user) {
        // Câu lệnh 2: Dự phòng (không kiểm tra status) - BẠN PHẢI THÊM 'role' VÀO ĐÂY
        $sql = 'SELECT id, username, name, avatar, email, password_hash, role 
                FROM users 
                WHERE email = :email
                LIMIT 1';
