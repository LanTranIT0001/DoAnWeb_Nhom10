...
class Pin extends Model
{
    ...
    /**
     * @return string|null image_url if pin exists and has at least one PENDING report
     */
    public function findImageUrlForReportedPin(int $pinId): ?string
    {
        if ($pinId <= 0) {
            return null;
        }
        ...
        $sql = 'SELECT p.image_url
                FROM pins p
                INNER JOIN pin_reports pr ON pr.pin_id = p.id AND pr.status = :status_pending
                WHERE p.id = :pin_id
                LIMIT 1';
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
...
// Trong file app/models/Pin.php
public function rejectReportsByPin(int $pinId): bool
{
    // Cập nhật tất cả báo cáo của Pin này về trạng thái REJECTED
    $sql = 'UPDATE pin_reports SET status = "REJECTED" WHERE pin_id = :pin_id AND status = "PENDING"';
    $stmt = $this->db->prepare($sql);
    return $stmt->execute(['pin_id' => $pinId]);
}
