...
class Message extends Model
{
    ...
    {
        $sql = 'SELECT m.id, m.sender_id, m.content, m.created_at, m.shared_pin_id, p.title AS shared_pin_title, p.image_url AS shared_pin_image, p.source_link AS shared_pin_source_link, u.name AS sender_name, u.avatar AS sender_avatar
                FROM messages m
    ...
    /**
     * Tổng số tin nhắn chưa đọc (từ người khác) trong mọi cuộc trò chuyện của user.
     */
    public function countUnreadMessagesForUser(int $userId): int
    {
        $sql = 'SELECT COUNT(*) FROM messages m
                ...
                WHERE m.sender_id <> :not_user AND m.read_at IS NULL';
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
            'user_id' => $userId,
            'not_user' => $userId,
