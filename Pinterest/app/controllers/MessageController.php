...
class MessageController extends Controller
{
    public function inboxAction(): void
    {
        ...
        if ($activeConversationId > 0 && !$messageModel->isUserInConversation($activeConversationId, $userId)) {
            $activeConversationId = 0;
        }

        // Chỉ đánh dấu đã đọc khi user mở đúng cuộc trò chuyện (có conversation_id trên URL), không mặc định hội thoại đầu tiên.
        if ($activeConversationId > 0) {
            try {
                $messageModel->markConversationRead($activeConversationId, $userId);
                $conversations = $messageModel->listUserConversations($userId);
            } catch (PDOException $e) {
                // giữ danh sách cũ nếu lỗi
            }
        }
